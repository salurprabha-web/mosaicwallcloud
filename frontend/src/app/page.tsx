...
async function getSections(): Promise<Section[]> {
  try { const r = await fetch(`${backendUrl}/api/page-sections/home`, { next: { revalidate: 60 } }); return r.ok ? r.json() : []; } catch { return []; }
}
async function getSettings(): Promise<Settings> {
  try { const r = await fetch(`${backendUrl}/api/site-settings`, { next: { revalidate: 300 } }); return r.ok ? r.json() : {}; } catch { return {}; }
}
async function getRecentPosts(): Promise<Post[]> {
  try { const r = await fetch(`${backendUrl}/api/blog`, { next: { revalidate: 300 } }); return r.ok ? r.json() : []; } catch { return []; }
}
